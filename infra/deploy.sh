...
# deployed Lambda and frontend need (recommendation.json, frontend/web/*).
#
# First run ever: `./deploy.sh --guided` (walks you through stack name/region
# and writes samconfig.toml). Every run after that: `./deploy.sh`.
#
...
# repo root (so the Lambda can import the sibling `mining`/`bedrock`
...
# standard Python/pip build. data/output is pruned too: the mining cache is
# now read from S3 at call time (see api/handlers/_data.py), not bundled.
set -euo pipefail
cd "$(dirname "$0")"

sam build --template-file template.yaml

for fn in AskFunction; do
  rm -rf \
...
# 1. Mining cache -> DataBucket. AskFunction reads this at request time (see
#    api/handlers/_data.py) instead of it being bundled into the Lambda zip.
#    Regenerate if missing/stale -- requires data/raw/*.xlsx/csv locally
#    (gitignored, not in the repo -- see README.md's data-layer section).
if [[ ! -f data/output/recommendation.json ]]; then
  echo "data/output/recommendation.json missing -- running mining pipeline..."
  python3 -m mining.co_occurrence
fi
aws s3 cp data/output/recommendation.json "s3://$DATA_BUCKET/recommendation.json"
